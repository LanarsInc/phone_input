...
class CountryFinder {
  ...
  void _filterByCountryCallingCode(String countryCallingCode) {
    int getSortPoint(Country country) => country.countryCode == countryCallingCode ? 1 : 0;

    _filteredCountries = _allCountries.where((country) => country.countryCode.contains(countryCallingCode)).toList()
  ...
  void _filterByName(String searchTxt) {
    ...
    int getSortPoint(String name, IsoCode isoCode) {
      bool isStartOfString = name.startsWith(searchTxt) || isoCode.name.toLowerCase().startsWith(searchTxt);
      return isStartOfString ? 1 : 0;
    }

    int compareCountries(Country a, Country b) {
      final sortPoint = getSortPoint(b.name, b.isoCode) - getSortPoint(a.name, a.isoCode);
    ...
    _filteredCountries = _allCountries.where((country) {
      ...
      return countryName.contains(searchTxt) || country.isoCode.name.toLowerCase().contains(searchTxt);
    }).toList()
